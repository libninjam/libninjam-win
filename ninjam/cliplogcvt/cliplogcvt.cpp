...
//                printf("BPI/BPM changed from %d/%.2f to %d/%.2f\n",m_cur_bpi,m_cur_bpm,bpi,bpm);
          ...
          case 2: // user
            ...
            {
#define NIX_KLUDGE
// this file compiles on nix and creates wave files successfully
//     but clipsort.lof and clipsort.txt are created always empty
//     and is otherwise buggy (see below)
// has potential but probably needs some TLC to be viable cross-platform
// KLUDGE: LineParser::getnumtokens() is buggy (at least on linux)
//             always fails on first accepted 'user' line
#ifdef NIX_KLUDGE
